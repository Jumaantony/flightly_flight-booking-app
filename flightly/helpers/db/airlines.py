...
class _Constants(object):
    ...
    '''
    _HEADER = {'User-Agent':
               'CORGIS Airlines library for educational purposes'}
...
if not _os.access(_Constants._DATABASE_NAME, _os.F_OK):
    raise DatasetException(
        "Error! Could not find a \"{0}\" file. Make sure that there is a \"{0}\" in the same directory as \"{1}.py\"! Spelling is very important here.".format(
            _Constants._DATABASE_NAME,
            __name__))
elif not _os.access(_Constants._DATABASE_NAME, _os.R_OK):
    raise DatasetException(
        "Error! Could not read the \"{0}\" file. Make sure that it readable by changing its permissions. You may need to get help from your instructor.".format(
            _Constants._DATABASE_NAME,
            __name__))
...
class _Auxiliary(object):
    ...
    def _byteify(input):
        ...
        if isinstance(input, dict):
            return {_Auxiliary._byteify(key): _Auxiliary._byteify(
...
def get_reports(test=False):
    ...
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM airlines LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM airlines".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
